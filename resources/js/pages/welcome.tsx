    ...
    return (
        <div className="min-h-screen bg-white text-green-700">
            ...
            <header className="bg-green-700 text-white p-6 flex justify-between items-center shadow-md">
                <div className="flex items-center gap-2">
                    <svg width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <text x="50%" y="55%" textAnchor="middle" fill="white" fontSize="18" fontWeight="bold" dy=".3em">UP</text>
                    </svg>
            ...
            <section className="bg-gradient-to-r from-green-600 to-green-400 text-white py-16 px-4 text-center">
                <h1 className="text-5xl md:text-6xl font-extrabold mb-4">Trouvez votre pharmacie</h1>
                <p className="text-xl md:text-2xl mb-6">
                    Commandez vos médicaments en toute simplicité, livraison rapide et sécurisée.
                </p>
            </section>

            {/* Pharmacies grid */}
            <main className="max-w-5xl mx-auto py-10 px-4">
                <h2 className="text-3xl font-bold mb-8 text-green-700">Pharmacies disponibles</h2>
                {pharmacies && pharmacies.length > 0 ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
                        {pharmacies.map((pharmacy: any) => (
                            <div key={pharmacy.id} className="bg-white border border-green-100 rounded-xl shadow hover:shadow-lg transition-shadow p-6 flex flex-col items-start">
                                ...
                                <div className="text-gray-600 mb-2">
                                    <span className="font-medium">Adresse : </span>{pharmacy.address || 'Non renseignée'}
                                </div>
                                <div className="text-gray-600">
                                    <span className="font-medium">Téléphone : </span>{pharmacy.phone || 'Non renseigné'}
                                </div>
                            </div>
                        ))}
                    </div>
                ) : (
                    <div className="text-center text-gray-500">Aucune pharmacie disponible pour le moment.</div>
                )}
            </main>
        </div>
    );
