...
export default function PharmacyProducts() {
    ...
    const handleCreateProduct = (e: React.FormEvent) => {
        ...
        router.post('/products', newProduct, {
            onSuccess: () => setNewProduct({ name: '', price: 0, stock: 0, pharmacy_id: '' }),
        });
    };
    ...
    return (
        <div className="min-h-screen bg-white text-black">
            ...
            <div className="p-6">
                ...
                <form onSubmit={handleCreateProduct} className="mb-8 rounded border border-gray-300 bg-gray-50 p-4">
                    ...
                    <button type="submit" className="mt-4 rounded bg-green-600 px-6 py-2 text-white transition hover:bg-green-700">
                        Ajouter
                    </button>
                </form>
                ...
                <div className="hidden md:block">
                    <table className="w-full border-collapse border border-gray-300">
                        ...
                        <tbody>
                            {products
                                .filter((p: any) => myPharmacies.some((ph: any) => ph.id === p.pharmacy_id))
                                .map((product: any) => (
                                    <tr key={product.id} className="hover:bg-gray-50">
                                        <td className="border border-gray-300 px-4 py-2">{product.id}</td>
                                        <td className="border border-gray-300 px-4 py-2">{product.name}</td>
                                        <td className="border border-gray-300 px-4 py-2">{product.pharmacy?.name}</td>
                ...
                <div className="space-y-4 md:hidden">
                    {products
                        .filter((p: any) => p.user_id === auth.id)
                        .map((product: any) => (
                            <div key={product.id} className="rounded border border-gray-300 bg-gray-50 p-4 shadow-sm">
                                <p>
                                ...
                                <p>
                                    <span className="font-semibold">Pharmacie :</span> {product.pharmacy?.name}
                                </p>
                                <p>
